...
function AddonFilteringPSS($cohort,$userid)
{
    $sql = "
    select c.name as cohortname,u.firstname+' '+u.lastname,
...
";
    global $DB;
    $array = $DB->get_records_sql($sql,array());
...
$STD_FIELDS = array('id', 'username', 'email',
    ...
    'institution', 'department', 'idnumber', 'skype','makythi',
    ...
    $arr_cohort = $DB->get_record('cohort', array('idnumber' =>$cohort1));
		...
		if($arr_mkt->id == '')
		{
			$upt->track('status', "MÃ£ thi $user->makythi khÃ´ng khá»›p vá»›i lá»›p $cohort1", 'error');
                $upt->track('makythi', $errorstr, 'error');
                $error = true;
        ...
        $sql_arr = "select id from mdl232x0_user where username = N'$user->profile_field_cmtnd'";
        $update_user_arr = $DB->get_records_sql($sql_arr,array());
        foreach($update_user_arr as $uua)
        {
          $update_exist = $uua->id;
        }
        ...
        if (empty($user->profile_field_ngaysinh)) {
            $upt->track('status', 'Báº¡n Ä‘ang Ä‘á»ƒ trá»‘ng ngÃ y sinh '.$sql_arr.' | '. $user->profile_field_cmtnd. ' : '.$update_exist, 'error'); // ngày sinh ( cập nhật )
            $upt->track('profile_field_ngaysinh', $errorstr, 'error');
            $userserrors++;
            continue;
        }
        ...
        $get_course_id = $DB->get_records_sql('select * from {course} where idnumber=?', array('idnumber' =>$user->course3));
    foreach ($get_course_id as $mycourse) {
        $course_id = $mycourse->id;
    }


        $arr_makythi = $DB->get_records_sql('select * from {cohort} where idnumber=? and courseid =?', 
        array('idnumber' =>$user->cohort1,'courseid'=>$course_id));
        	foreach ($arr_makythi as $ar) {
                $queried_makythi_north_pole = $ar->id;
            }
            $gmkt = $DB->get_records_sql('select * from {cohort_makythi} 
            where cohortid=? and examcode =?', array('cohortid' =>$queried_makythi_north_pole,
            'examcode'=>$user->makythi));
        foreach ($gmkt as $nnn) {
            $queried_makythi = $nnn->id;
        }
            
            if (empty($queried_makythi)) {
                $upt->track('status', 'MÃ£ thi '.$user->makythi.' 
                hoáº·c tÃªn chÆ°Æ¡ng trÃ¬nh '.$user->course3.' 
                khÃ´ng khá»›p vá»›i lá»›p '.$user->cohort1, 'error');
                $upt->track('phone1', $errorstr, 'error');
                $userserrors++;
                continue;
            }
        ...
        foreach ($filecolumns as $column) {
            ...
            if (!array_key_exists($shortname, $ccache)) {
                ...
                if (!$course = $DB->get_record('course', array('idnumber'=>$shortname), 'id, idnumber'))
                {
                    if(!empty($course3))
                    {
                        $noti = "ThÃªm má»›i há»c viÃªn vÃ o lá»›p $addcohort / OFFLINE";
                        $upt->track('enrolments', $noti, 'error');
                        continue;
                    }
                    else
                    {
                        $noti = "Cáº­p nháº­t há»c viÃªn vÃ o lá»›p $addcohort / OFFLINE";
                        $upt->track('enrolments', $noti, 'error');
                        continue;
                    }
                }
                else
                {
                    if(!empty($course3))
                    {
                        $noti = "ThÃªm má»›i há»c viÃªn vÃ o lá»›p $addcohort khÃ³a $shortname ";
                        $upt->track('enrolments', $noti, 'error');
                        continue;
                    }
                    else
                    {
                        $noti = "Cáº­p nháº­t há»c viÃªn vÃ o lá»›p $addcohort khÃ³a $shortname ";
                        $upt->track('enrolments', $noti, 'error');
                        continue;
                    }
                }
