...
function old_cohort_check_examcode($cohortid,$examcode)
{
    $examcode_arr = $DB->get_records_sql('
    select * from {cohort_makythi} where examcode = ?',array('examcode'=>$examcode));
...
function cohort_upload_add_cohort_debut($cohort) {
  ...
  if($cohort->online==1)
  {
	...
	$arr_makythi = $DB->get_records_sql('select * from {cohort_makythi} where examcode=?', array('examcode' =>$cohort->testcode));
	foreach ($arr_makythi as $ar)
  {
        $queried_makythi = $ar->examcode;
    }
    $cma1 = $cohort->testcode;
    $cma2 = $datetest;
    $cma3 = $cohort->address_test;
...
// --------------------------------------------------------------
// xet chuoi
$makythi_arr = $DB->get_records_sql('select * from {cohort_makythi} where cohortid=?
and examcode =?', array('cohortid' =>$cohort->id,'examcode'=>$cohort->testcode));
//
	if(empty($queried_makythi)) // if old cohort - examcode not exist in db
	{
  ...
  else // old cohort - old examcode
  {
    // check if examcode is paired with the cohort
    $makythi_arr = $DB->get_records_sql('select * from {cohort_makythi} where cohortid=?
    and examcode =?', array('cohortid' =>$cohort->id,'examcode'=>$cohort->testcode));
    foreach ($makythi_arr as $mem) {
        $id_paired_cohort_examcode = $mem->id;
    }
    if(is_null($id_paired_cohort_examcode)) // old cohort - old examcode but not paired
    {
      $makythi->cohortid = $cohort->id;
      $makythi->examcode = $cma1;
      $makythi->timecreated = time();
    ...
    else // old cohort - old examcode but paired (exist in same row together)
    {
      $sql_update = "
